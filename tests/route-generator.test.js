...
describe('generateRoute', () => {
  ...
  it('creates the controller file', async () => {
    ...
    expect(
      await fs.pathExists(path.join(tmpDir, 'src', 'controllers', 'users.js')),
    ).toBe(true);
  });
  ...
  it('creates the service file', async () => {
    await generateRoute('users', tmpDir);
    expect(
      await fs.pathExists(path.join(tmpDir, 'src', 'services', 'users.js')),
    ).toBe(true);
  });
  ...
  it('creates the model file', async () => {
    await generateRoute('users', tmpDir);
    expect(
      await fs.pathExists(path.join(tmpDir, 'src', 'models', 'users.js')),
    ).toBe(true);
  });
  ...
  it('creates the route index file', async () => {
    ...
    expect(
      await fs.pathExists(
        path.join(tmpDir, 'src', 'routes', 'users', 'index.js'),
      ),
    ).toBe(true);
  });
  ...
  it('creates the test file', async () => {
    ...
    expect(
      await fs.pathExists(
        path.join(tmpDir, 'tests', 'routes', 'usersRoute.test.js'),
      ),
    ).toBe(true);
  });
  ...
  it('controller references the service', async () => {
    ...
    const content = await fs.readFile(
      path.join(tmpDir, 'src', 'controllers', 'users.js'),
      'utf-8',
    );
